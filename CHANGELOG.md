...
- Fixed: 安全网超时后仍恢复更新竞态（P1）——`stopBackend` 现在返回**是否真正退出**（`Promise<boolean>`）；更新流程在旧进程仍存活时**中止本次更新**并提示（不启动替代进程，避免端口竞态与后续误判崩溃）；POSIX 信号测试在 Windows CI 上跳过（`taskkill /F` 语义不同），另加 win32 分支测试（taskkill 或 spawn 失败回退）。
- Fixed: 系统空闲无条件刷新健康页面（P2）——半开事件流从页面外部**无可靠探测手段**（后端健康只能证明 HTTP 可达），按审查建议将 `idleReloadMinutes` **默认改为 `0`（关闭）**，需要时显式开启；开启后仍保留"后端健康"门槛作为额外保险，另增每 60 秒的渲染进程存活探测（`executeJavaScript` 2s 超时）恢复挂死的渲染进程。
- Fixed: 无效空闲配置导致 NaN 每分钟刷新（P2）——`idleReloadMinutes` 用 `Number.isFinite` 校验，非法值回退默认 `0`。
- Fixed: 唤醒时可能连续刷新两次（P2）——空闲重载改走 `scheduleReload`（与 resume/GPU 重载合并防抖）。
- Added: 单元测试 6 个（NaN 配置、stopBackend 四态含布尔返回），共 52 个。
